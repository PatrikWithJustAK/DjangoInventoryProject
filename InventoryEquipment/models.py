...
class Transaction(models.Model):
    ...
    date_submitted = models.DateTimeField(_("When was this Transaction Submitted?"), auto_now_add=True)
    ...
    serial_number = models.ForeignKey(EquipmentInstance, verbose_name=("The equipment that this transaction is for"), on_delete=models.CASCADE, blank=True)
    submitter= models.ForeignKey(CustomUser, related_name="requester", verbose_name=_(""), on_delete=models.CASCADE)
    store_location = models.ForeignKey("InventoryStores.StoreLocation", verbose_name=("Location Equipment is currently at"), on_delete=models.CASCADE)
    location_status = models.CharField(_("Location Status Of Equipment"), max_length=3, choices=LOCATION_STATUS, default='OH')
    equipment_status = models.CharField(_("Equipment Status active/inactive/etc"), max_length=2, choices =EQUIPMENT_STATUS, default='A')
    current_customer_id = models.CharField(_("Customer ID of the customer who currently has possession of equipment"), max_length=50)
    meter_hours = models.PositiveIntegerField(_("Meter hours on the equipment"), null=True)
